...
public class BossBehaviour : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (HP <= 0 && phasesLeft < 0)
        {
            ...
            GameObject.Find("KosuzuController").GetComponent<LeaveFromStage>().canLeave = true;
        }
        ...
        if (GameObject.Find("BossController").GetComponent<Animator>().GetBool("Battle Ongoing?"))
        {
            ...
            switch (boss)
            {
                case Boss.ChaseScene:
                    break;
                case Boss.Reimu:
                    ReimuAttacks(emitter.GetComponent<DanmakuEmitter>(), phasesLeft);
                    break;
                case Boss.Yukari:
    ...
    void ReimuAttacks(DanmakuEmitter emitter, int phasesLeft)
    {
        // NS, S, NS, S, S, LW
        Color temp;
        ColorUtility.TryParseHtmlString("#FF61AE", out temp);
        int phase = 4 - phasesLeft;
        switch (phase)
        {
            case 1:
                // Non-spell
                ...
                emitter.Speed = 4.5f;
                emitter.AngularSpeed = 0f+spellTimer/100f;
